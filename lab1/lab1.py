...
same_identitiy_rule = IF(OR("male (?x)", "female (?x)"), THEN("(?x) is (?x)"))
sibling_rule = IF(AND("parent (?x) (?y)", "parent (?x) (?z)", NOT("(?y) is (?z)")), THEN("sibling (?y) (?z)", "sibling (?z) (?y)"))
brother_rule = IF(AND("sibling (?x) (?z)", "male (?x)"), THEN("brother (?x) (?z)"))
sister_rule = IF(AND("sibling (?x) (?z)", "female (?x)"), THEN("sister (?x) (?z)"))
...
cousin_rule = IF(AND("parent (?x) (?y)", "parent (?z) (?t)", "sibling (?x) (?z)"), THEN("cousin (?y) (?t)", "cousin (?t) (?y)"))
grand_rule = IF(AND("parent (?x) (?y)", "parent (?z) (?x)"), THEN("grandparent (?z) (?y)", "grandchild (?y) (?z)"))

family_rules = [same_identitiy_rule, sibling_rule, brother_rule, sister_rule,
                mother_rule, father_rule, daughter_rule, son_rule, cousin_rule, grand_rule]
...
HOW_MANY_HOURS_THIS_PSET_TOOK = 'Many'
WHAT_I_FOUND_INTERESTING = "I liked this lab generally."
